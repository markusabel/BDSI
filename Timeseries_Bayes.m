%% Dynamical system identification via sparsity-enforcing Bayesian inference 
...
% for the identification of a dynamical system from a time series. 
...
% dynamical system. A number of 2D, 3D, 4D dynamical systems are implemented
%
...
addpath('dyn_sys_models')  %Niven model library 
...
if timeseries == 1 
    %tstep=0.05;   %
    tstep=0.01   %Lorenz, Shilnikov, Chenueta B
    %tstep=0.02  %Vance, others, 0.05 FeIIFeIII
...
%eps = 0.1; % noise, 
eps = 0.2; % noise, Lorenz, Shilnikov
...
dirname = ['ex_', eqname, '_dim', num2str(n,1), '_ep', num2str(eps,3),...
    ...
    '_t', num2str(tfinal,4), '_tstep', num2str(tstep,4)]
...
str0a=[fighead2];
str1=['Added Gaussian noise: \epsilon=' num2str(eps,3)];
...
figure
    annotation('textbox',...
    [0.05 0.05 0.9 0.55],...
    'String',{str0, str0a, str1, str2, str5, str6, str7, str8, str9, ...
       str10, str11, str12},...
    'FontSize',14,...
    'FontName','Helvetica');  
